...
namespace Lab3Matan
{
    public partial class Form1 : Form
    {
        ...
        public Form1()
        {
            ...
            PowerTextBox.KeyDown += PowerTextBox_KeyDown;
        ...
        private void LagrangeButton_CheckedChanged(object sender, EventArgs e)
        {
            ...
            if (LagrangeButton.Checked)
            {
                ...
                LagrangeInterpolationMethod lagrange = new LagrangeInterpolationMethod(x, y);
                for (float xi = 1; xi <= 8; xi += 0.01f)
                {
                    float interpolatedValue = lagrange.GetFunctionValue(xi);
                    xs.Add(xi);
                    ys.Add(interpolatedValue);
                }
                ...
                sp1.Label = "Многочлен Лагранжа";
                sp1.LineWidth = 1;

                sp1.MarkerSize = 2;
                formsPlot1.Plot.ShowLegend();
                formsPlot1.Plot.Add.Markers(x, y, ScottPlot.MarkerShape.FilledCircle, 10, ScottPlot.Color.FromHex("#FF00EC"));
        ...
        private void NewtonButton_CheckedChanged(object sender, EventArgs e)
        {
            if (NewtonButton.Checked)
            {
                ...
                NewtownInterpolationMethod newtown = new NewtownInterpolationMethod(x, y);
                for (float xi = 1; xi <= 7.1; xi += 0.01f)
                {
                    float interpolatedValue = newtown.GetFunctionValue(xi);
                    xs1.Add(xi);
        ...
        private void SquareButton_CheckedChanged(object sender, EventArgs e)
        {
            if (SquareButton.Checked)
            {
                ...
                SmoothingMethod smoothing = new SmoothingMethod(x, y, power);
                for (float xi = 0; xi <= 8; xi += 0.05f)
                {
                    float interpolatedValue = smoothing.GetFunctionValue(xi);
                    xs1.Add(xi);
                    ys1.Add(interpolatedValue);
                ...
                switch (power)
                {
                    case 1:
                        ...
                        DrawGraphic(xs1, ys1, ScottPlot.Colors.Green, "Степень 1");
                        break;
                    case 2:
                        DrawGraphic(xs1, ys1, ScottPlot.Colors.Purple, "Степень 2");
                        break;
                    case 3:
                        DrawGraphic(xs1, ys1, ScottPlot.Colors.Red, "Степень 3");
                        break;
                    case 4:
                        DrawGraphic(xs1, ys1, ScottPlot.Colors.Blue, "Степень 4");
                        break;
                    default:
                        formsPlot1.Plot.Add.Scatter(xs1, ys1, ScottPlot.Color.FromHex("#0AD52F"));
